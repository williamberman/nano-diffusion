...
def test_train_unet_inpainting_cross_attention_conditioning_log_validation():
    print("test_train_unet_inpainting_cross_attention_conditioning_log_validation")

    x = train.init_train_unet_inpainting_cross_attention_conditioning(
        namedtuple_helper(
            controlnet_resume_from=None,
            use_8bit_adam=True,
            optimizer_resume_from=None,
    ...
    output_images, conditioning_images = train.log_validation_train_unet_inpainting_cross_attention_conditioning(
        tokenizer_one=tokenizer_one,
        text_encoder_one=text_encoder_one,
        tokenizer_two=tokenizer_two,
        text_encoder_two=text_encoder_two,
...
def test_train_unet_inpainting_cross_attention_conditioning_main():
    print("test_train_unet_main")

    with tempfile.TemporaryDirectory(dir=os.environ.get("TMP_DIR", None)) as tmpdir:
        training_config = train.TrainingConfig(
            output_dir=tmpdir,
            train_type="unet_inpainting_cross_attention_conditioning",
            learning_rate=0.00001,
            gradient_accumulation_steps=1,
            mixed_precision=torch.float16,
            max_train_steps=2,
...
if __name__ == "__main__":
    ...
    if test_train:
        ...
        test_train_unet_inpainting_cross_attention_conditioning_log_validation()
        test_train_unet_inpainting_cross_attention_conditioning_main()
